"""
Training file for the first half of LEQWENDY method
"""
from models import Leqwendy_first_half
...
model = Leqwendy_first_half().to(device)
...
for epoch in range(num_epochs):
    ...
    for k in range(100):
        ...
        for i in range(10):            
            ...
            total_auroc += t1 / 1000 # evaluate model performance
            total_auprc += t2 / 1000
...
torch.save(model.state_dict(), "weights/leqwendy_1st.pth")
...
for i in data_ranges:
    ...
    np.save(f'training_data/dataset_{ds_num}_leqwendyA.npy', qwendy_A)   
...
for k in range(100):
    ...
    for j in range(10):            
        qwendy_A[j+k*10] = Qwendy_Kx(kstar[j], x[j+k*10])
np.save('training_data/dataset_val_leqwendyA.npy', qwendy_A)  
